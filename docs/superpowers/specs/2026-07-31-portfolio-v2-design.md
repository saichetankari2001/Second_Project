...
- **3D hero and background (revised post-launch):** the first implementation carried forward a pre-existing hand-rolled Three.js "coil" scene, recolored but not actually rebuilt — it read as a flat wireframe with no real bloom, nothing like the approved prototype. Replaced with genuine components from the **React Bits** library (`github.com/DavidHDev/react-bits`), vendored into `src/vendor/react-bits/` and recolored to the site's teal/green palette: **Orb** (a reactive glowing WebGL sphere, via `ogl`) as the hero's centerpiece, and **Aurora** (a WebGL shader aurora effect) for the page-wide ambient background, replacing hand-rolled CSS blobs. This dropped three.js/@react-three/fiber/@react-three/postprocessing entirely — the hero's JS chunk went from 954KB to 8.6KB. Orb accepts the same `pointerRef` the device-orientation hook already writes to, so mouse and mobile-tilt reactivity carry over unchanged.
...
- **Linked to its real repo** (revised post-launch): the first version rendered this as a static, non-clickable card — inconsistent with every other project card, which links out to GitHub. Now wraps the whole tile as a link to `github.com/saichetankari2001/tjs-kebab-centre`, matching the interaction pattern everywhere else.
...
**Curated detail per card (revised post-launch):** GitHub's raw repo `description` field was thin or empty (`booking-api`'s was blank) and read as noticeably less detailed than the hand-written featured tile — a jarring inconsistency. Added `src/projectDetails.js`, a curated `{ summary, highlights[] }` override keyed by repo name for the three known auto-synced projects, rendered as the same summary + bullet-highlights format as the featured tile. A repo not yet in that map still renders (falls back to GitHub's raw description, no highlights) — new pushes keep showing up automatically, they just read plainer until curated.
