...
**Explicitly excluded from the auto-sync grid:** the portfolio repo itself (`Second_Project`) and `Chintu1112` (personal, non-portfolio project) — excluded by an explicit denylist of repo names in the fetch logic, since GitHub's public API would otherwise surface both. SACA and the Multi-Cloud Photo Album project (also on the resume) are explicitly **out of scope** for this build per direct instruction — not added now, can be a future follow-up.
